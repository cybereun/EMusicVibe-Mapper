...
const App: React.FC = () => {
  ...
  const checkKey = async () => {
    ...
    // 2. Check LocalStorage Fallback (Standalone)
  ...
  return (
    <div className="min-h-screen bg-[#020617] text-slate-100 flex flex-col font-sans">
      ...
      <header className="py-4 px-6 md:px-12 border-b border-slate-900 flex justify-between items-center bg-[#020617]/90 backdrop-blur-xl sticky top-0 z-50">
        ...
        <div className="flex items-center space-x-4">
          ...
          <button 
            ...
            className="p-2 bg-slate-900/50 hover:bg-slate-800 rounded-xl transition-all border border-slate-800 text-slate-400 hover:text-white"
          >
      ...
      {/* Settings Modal - BYOK Implementation */}
      {isSettingsOpen && (
            ...
            <div className="space-y-6">
              <div className="space-y-3">
                ...
                <div className="flex space-x-2">
                  <input 
                    ...
                    className="flex-grow bg-slate-950 border border-slate-800 rounded-2xl px-5 py-4 text-sm focus:ring-2 focus:ring-amber-500/50 outline-none text-slate-100"
                  />
                  <button 
                    onClick={handleSaveManualKey}
                    className="bg-amber-500 text-slate-950 px-6 py-2 rounded-2xl text-xs font-bold hover:bg-amber-400 transition-all"
                  >
                    Save
                  </button>
                </div>
                <p className="text-[10px] text-slate-600 leading-relaxed italic">
                  * Stored in your browser's local storage for private access.
                </p>
              </div>
              ...
              {window.aistudio && (
                <div className="pt-6 border-t border-slate-800">
                  <button 
                    ...
                    className="w-full py-4 bg-slate-800 border border-slate-700 text-white rounded-2xl text-sm font-bold hover:bg-slate-700 transition-all"
                  >
                    Open Google Picker
                  </button>
                </div>
              ...
              <div className="space-y-4 pt-6 border-t border-slate-800">
                <button 
                  ...
                  className="w-full py-4 bg-white text-slate-950 hover:bg-slate-200 disabled:bg-slate-800 disabled:text-slate-600 rounded-2xl font-bold transition-all"
                >
                  {isTestingConn ? "Verifying..." : "Verify Connection"}
                </button>
                
                {testResult && (
                  <div className={`p-4 rounded-xl text-xs border ${testResult.success ? 'bg-emerald-500/10 border-emerald-500/20 text-emerald-400' : 'bg-red-500/10 border-red-500/20 text-red-400'}`}>
                    {testResult.message}
                  </div>
                )}
              </div>
      ...
      <main className="flex-grow container mx-auto px-4 py-8 md:py-16 max-w-5xl">
        ...
        {error && (
          <div className="bg-red-500/10 border border-red-500/20 text-red-400 p-5 rounded-2xl mb-12 text-center animate-fadeIn">
            {error}
          </div>
        )}
               ...
               <div className="mt-16 max-w-md mx-auto">
                 <div className="relative bg-slate-900 border border-slate-800 rounded-3xl p-1.5 flex items-center shadow-2xl">
                   <input 
                      ...
                      className="flex-grow bg-transparent border-none text-white px-5 py-4 focus:ring-0 outline-none text-sm placeholder:text-slate-600"
                   />
                   <button 
                     onClick={handleCustomDestination}
                     className="bg-amber-500 text-slate-950 font-black px-6 py-3 rounded-2xl shadow-lg hover:bg-amber-400 transition-all"
                   >
                     GO
                   </button>
                 </div>
